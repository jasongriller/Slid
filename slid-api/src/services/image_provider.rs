...
impl UnsplashOpenAiProvider {
    pub fn from_env() -> Self {
        Self {
            unsplash_key: std::env::var("SLID_UNSPLASH_ACCESS_KEY").unwrap_or_default(),
            openai_key: std::env::var("SLID_OPENAI_API_KEY").unwrap_or_default(),
        }
    }
}
...
impl ImageProvider for UnsplashOpenAiProvider {
    async fn search(&self, _query: &ImageQuery) -> ApiResult<ProvidedImage> {
        ...
        Err(ApiError::Upstream("Image search requires Unsplash API key configuration. Set SLID_UNSPLASH_ACCESS_KEY in .env".into()))
    }
    ...
    async fn generate(&self, _query: &ImageQuery) -> ApiResult<ProvidedImage> {
        ...
        Err(ApiError::Upstream("Image generation requires OpenAI API key configuration. Set SLID_OPENAI_API_KEY in .env".into()))
    }
}
