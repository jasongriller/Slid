# Slid Design Philosophy
Slid exists because most AI slide tools produce slop. They use 3–4 layouts
on repeat, generate text first and stuff it into a layout afterwards, and have
no real design system. This doc is the contract that prevents Slid from
...
2. **Tokens are the only legal styling primitives.** Typography sizes, spacings,
   colors, radii, and shadows all come from `@slid/schema`'s tokens.
...
1. Add a Zod variant in `slid-schema/src/schema/index.ts` with strict
   max-lengths and item caps.
2. Register the layout in `slid-schema/src/layouts/index.ts` with
   `suitableFor`, `supportsImage`, `supportsAccent`, and any caps.
3. Build the Vue component in `slid/src/components/slides/<Name>.vue`.
...
6. Add at least one test case in `slid-schema/test/schema.test.ts` and
...
- All `@slid/schema` tests pass with 100% coverage.
