...
use crate::services::ai_orchestrator::{AiOrchestrator, Brief, RegenerateRequest, SuggestImprovementsRequest};
...
#[post("/ai/suggest-improvements", data = "<request>")]
async fn suggest_improvements(
    _user: AuthUser,
    request: Json<SuggestImprovementsRequest>,
    orchestrator: &State<Box<dyn AiOrchestrator>>,
) -> ApiResult<Json<serde_json::Value>> {
    let suggestions = orchestrator.suggest_improvements(&request).await?;
    Ok(Json(suggestions))
}

pub fn routes() -> Vec<Route> {
    routes![generate_deck, regenerate_slide, suggest_improvements]
}
...
mod tests {
    ...
    fn token() -> String {
        std::env::set_var("SLID_JWT_SECRET", "ai-test");
