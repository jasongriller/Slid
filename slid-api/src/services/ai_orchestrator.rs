...
impl OpenAiOrchestrator {
    pub fn from_env() -> Self {
        Self {
            api_key: std::env::var("SLID_OPENAI_API_KEY").unwrap_or_default(),
        }
    }
}
...
impl AiOrchestrator for OpenAiOrchestrator {
    async fn generate_deck(&self, _brief: &Brief) -> ApiResult<Deck> {
        if self.api_key.is_empty() {
            return Err(ApiError::AiFailure(
                "SLID_OPENAI_API_KEY is not set".to_string(),
            ));
        }
        ...
        Err(ApiError::AiFailure("AI deck generation requires OpenAI API key configuration. Set SLID_OPENAI_API_KEY in .env".into()))
    }
    ...
    async fn regenerate_slide(
        ...
        Err(ApiError::AiFailure("AI slide regeneration requires OpenAI API key configuration. Set SLID_OPENAI_API_KEY in .env".into()))
    }

    async fn suggest_improvements(
        &self,
        _request: &SuggestImprovementsRequest,
    ) -> ApiResult<serde_json::Value> {
        if self.api_key.is_empty() {
            return Err(ApiError::AiFailure(
                "SLID_OPENAI_API_KEY is not set".to_string(),
        ...
        // TODO: Implement AI suggestions using OpenAI API
        // For now, return stub suggestions to enable frontend development
        let suggestions = serde_json::json!({
            "slideIndex": _request.slide_index,
            "suggestions": [
                {
                    "id": "layout-1",
                    "type": "layout",
                    "title": "Consider Split Layout",
                    "description": "This slide has a lot of content that could be better organized with a split-screen layout.",
                    "priority": "medium",
                    "action": {
                        "recommendedLayout": "split-hero",
                        "reason": "Better content hierarchy and improved visual balance"
                    }
                },
                {
                    "id": "content-1",
                    ...
                    "description": "Some of your text could be more concise.",
                    "priority": "low",
                    "action": {
                        "changes": [
                            "Reduce title to under 10 words",
...
mod tests {
    ...
    async fn openai_orchestrator_errors_without_key() {
        std::env::remove_var("SLID_OPENAI_API_KEY");
